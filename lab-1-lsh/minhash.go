...
func shingleHashes(text string, shingleSize int) map[uint64]struct{} {
	...
	for i := 0; i+shingleSize <= len(tokens); i++ {
		...
		set[hashString64(shingle)] = struct{}{}
	}
	return set
}
...
func (idx *Index) Add(doc Document) error {
	...
	idx.docs[doc.ID] = storedDocument{Text: doc.Text, Signature: signature, Shingles: shingles}
...
func (idx *Index) FindDuplicates(text string, threshold float64) []Match {
	...
	candidates := idx.CandidateIDs(text)
...
func jaccard(left, right map[uint64]struct{}) float64 {
